...
func (h *Handler) Get(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response.Error(w, http.StatusNotFound, "NOT_FOUND", "song not found")
		return
	}
...
func (h *Handler) ListMine(w http.ResponseWriter, r *http.Request) {
	...
	if artistID == "" {
		response.Error(w, http.StatusBadRequest, "MISSING_ARTIST_ID", "artist_id query param is required")
		return
	}
	limit, _ := strconv.Atoi(r.URL.Query().Get("limit"))
	offset, _ := strconv.Atoi(r.URL.Query().Get("offset"))

	res, err := h.service.ListMine(r.Context(), artistID, limit, offset)
	if err != nil {
		response.Error(w, http.StatusInternalServerError, "LIST_FAILED", err.Error())
		return
	}
